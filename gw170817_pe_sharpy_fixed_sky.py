"""Parameter estimation of GW170817 with SHARPy — sky location fixed.

Same as gw170817_pe_sharpy_original.py but with ra and dec fixed to the
known host galaxy (NGC 4993) values:
    ra  = 3.44616 rad
    dec = -0.408084 rad

The sampler operates on a reduced 11-dimensional parameter space.  A
wrapper around the likelihood re-inserts the fixed ra/dec into the full
13-parameter vector expected by SHARPy internals.

Usage
-----
    python gw170817_pe_sharpy_fixed_sky.py
...
OUTDIR = "outdir_GW170817_sharpy_fixed_sky"
LABEL = "GW170817_sharpy_fixed_sky"
...
def main() -> None:
    ...
    # ── Wrapper: 11-param reduced vector → 13-param full vector ──
    # Reduced vector (what the sampler sees):
    #   [0] logdist, [1] incl, [2] phic, [3] pol,
    #   [4] mc, [5] q, [6] tc,
    #   [7] chi1, [8] chi2, [9] lambda_1, [10] lambda_2
    #
    ...
    def log_likelihood_reduced(params_11):
        """Insert fixed ra/dec and evaluate the full likelihood."""
    ...
    # ── 2. Define prior bounds (11 parameters) ───────────────────
    ...
    print(f"Sampling {len(parameter_names)} parameters (ra, dec fixed)")
