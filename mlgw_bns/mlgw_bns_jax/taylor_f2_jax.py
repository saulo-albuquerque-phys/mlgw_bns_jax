...
#@jax.jit
def decreasing_function(frequencies, merger_freq):
...
#        + decreasing_function(frequencies[mask_mid])
#        * connecting_coefficient
#    )

#    pn_amp[mask_end] = decreasing_function(frequencies[mask_end])

#    return pn_amp


#@jax.jit
def smoothly_connect_with_zero_jax(
    frequencies,
    pn_amp,
    pivot_1,
    pivot_2,
    ...
    # masks as booleans
    mask_mid = (pivot_1 <= frequencies) & (frequencies < pivot_2)
    mask_end = (frequencies >= pivot_2)

    # normalized x ∈ [0,1] for the smoothing region
    x = (frequencies - pivot_1) / (pivot_2 - pivot_1)

    connecting_coeff = smoothing_func(x)  # must be defined for ALL x

    # decreasing_function computed on all frequencies
    dec = decreasing_function(frequencies, merger_freq)

    # mid region blend
    blended_mid = pn_amp * (1 - connecting_coeff) + dec * connecting_coeff

    # Now assemble the final pn_amp using where(), preserving static shape
    pn_amp = np.where(mask_mid, blended_mid, pn_amp)
    pn_amp = np.where(mask_end, dec, pn_amp)
...
#@jax.jit
def amplitude_3h_post_newtonian_jax(frequencies, mass_ratio, chi_1, chi_2, lambda_1, lambda_2):
    par_dict = taylor_f2(frequencies, mass_ratio, chi_1, chi_2, lambda_1, lambda_2)
    ...
    pn_amp = (
        Af3hPN(
            ...
            par_dict["s2y"],
            par_dict["s2z"],
            lambdatilde(par_dict["q"], par_dict["lambda1"], par_dict["lambda2"]),
            dlambda(par_dict["q"], par_dict["lambda1"], par_dict["lambda2"]),
            Deff=par_dict["Deff"],
        )
        * taylor_f2_prefactor(eta(par_dict["q"]))
    )

    # merger_freq = frequency_of_merger(params)

    return smoothly_connect_with_zero_jax(frequencies, pn_amp, 0.01, 0.02, 0.02)

#@jax.jit
def phase_5h_post_newtonian_tidal_jax(frequencies, mass_ratio, chi_1, chi_2, lambda_1, lambda_2):
