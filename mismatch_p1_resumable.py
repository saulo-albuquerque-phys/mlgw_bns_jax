"""
Compute JAX vs Original mismatches in resumable batches.
Saves results every 50 waveforms.
"""
import warnings
...
N_WAVEFORMS = 1000
SAVE_FILE = "mismatches_jax_vs_original.npy"
...
for i in range(start_idx, N_WAVEFORMS):
    ...
    params_orig = ParametersWithExtrinsic(mass_ratio=q, lambda_1=l1, lambda_2=l2,
        chi_1=c1, chi_2=c2, distance_mpc=distance_mpc, inclination=inclination, total_mass=total_mass)
    try:
...
np.save(SAVE_FILE, np.array(mismatches))
print(f"Phase 1 complete! {len(mismatches)} mismatches.", flush=True)
