...
The relative-binning (RB) method of Zackay et al.~\cite{Zackay:2018}
...
$\Delta t = 1/f_s$ is the sampling interval, $N = f_s T$ is the number of
samples in the analysis segment of duration $T$, and $\overline{\cdot}$
denotes complex conjugation.  Given these pre-computed
...
\begin{equation}
    ...
    \sum_j r_j^{(0)} \tilde{A}_{0,j}^{(d)}\,
    e^{-\mathrm{i}2\pi f_j \Delta t_c},
\end{equation}
where $\tilde{A}_{0,j}^{(d)}$ is the $A_0$ array computed with the
fiducial phase set to zero (phase-stripped).  Integrating over $\varphi_c \in [0, 2\pi)$
with a uniform prior gives
\begin{equation}
    \mathcal{L}_{\rm marg}^{t_c,\varphi_c}
    \propto \exp\!\bigl(-\tfrac{2\Delta t}{N} \sum_d\sum_j |r_j^{(0)}|^2 B_{0,j}^{(d)}\bigr)\,
...
downloading 1024\,s of strain at 4096\,Hz and extracting a 128-s analysis
segment centered on the merger trigger time.  The setup is:
\begin{itemize}
